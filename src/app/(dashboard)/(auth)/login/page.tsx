      ...
      <CardContent>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handleLogin)}>
            <div className="grid w-full items-center gap-4">
              <div className="flex flex-col space-y-1.5">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-stone-800">Email <User size={15} /></FormLabel>
                      <FormControl>
            ...
            <Button type="submit" disabled={loading}> {loading ? <Loader className="animate-spin" size={15} /> : "Login" }</Button>
            <p className="text-sm">Doesn't have an account? <Link href="/register" className="text-indigo-600">Register</Link></p>
            </div>
          </form>
        </Form>
      </CardContent>
