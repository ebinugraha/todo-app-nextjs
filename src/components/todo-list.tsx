...
export const TodoList = ({todo}: {todo: Todo[]}) => {
    ...
    const { onOpen } = useUpdateTodoModal()
        ...
        <>
            <div>
                ...
                <div  className="pt-6 grid grid-cols-1 lg:grid-cols-[repeat(auto-fill,minmax(250px,1fr))] gap-4 mb-3">
                    {uncompletedTodo.map((item) => (<CardTodo todo={item} key={item.id} onClick={() => {}} onClickButton={(id) => handleToggleComplete(id)} onUpdate={(id) => {onOpen(id)}} onDelete={(id) => handleDelete(id)}/>))}
                </div>
            </div>
            <Separator/>
            <div>
